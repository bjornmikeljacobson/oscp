...
def sanitize_url(url):
    ...
    if FORCE_SSL:
        sanitized = parsed_url._replace(
            netloc=parsed_original_url.netloc, scheme="https"
        )
    else:
...
def dump_jobs(url):
    ...
    try:
        ...
        print(
            "[ERROR] This server refused our key negociation, retry with -d to downgrade ciphers"
        )
        exit(1)
    ...
    try:
        ...
        print(
            "[ERROR] Failed parsing server response for {}, try -f maybe or -v for debug".format(
                url + "/api/json/"
            )
        )
        if DEBUG:
    ...
    if "jobs" in response:
        for job in response["jobs"]:
            ...
            try:
                ...
                print(
                    "[ERROR] Gave up on job {} because of a timeout (server is probably busy)".format(
                        job["name"]
                    )
                )
...
def dump_build(url):
    ...
    consoleText = r.text
    r = SESSION.get(
        url + "/injectedEnvVars/api/json", verify=False, auth=AUTH, timeout=20
    )
    envVars = r.text
...
parser.add_argument(
    "-l", "--last", action="store_true", help="Dump only the last build of each job"
)
parser.add_argument(
    "-r",
    "--recover_from_failure",
    action="store_true",
    help="Recover from server failure, skip all existing directories",
)
parser.add_argument(
    "-d",
    "--downgrade_ssl",
    action="store_true",
    help="Downgrade SSL to use RSA (for legacy)",
)
parser.add_argument(
    "-f",
    "--force-ssl",
    action="store_true",
    help="Force usage of SSL if API returns wrong HTTP-only links",
)
parser.add_argument(
    "-s",
    "--no_use_session",
    action="store_true",
    help="Don't reuse the HTTP session, but create a new one for each request (for legacy)",
)
