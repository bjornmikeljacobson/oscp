...
print(
    bcolors.WARNING
    + "[+] Insert the target including the WordPress instance:"
    + bcolors.ENDC
)
...
try:
    ...
    print(
        bcolors.FAIL
        + "[X] Target not available. Please check the URL you've entered."
        + bcolors.ENDC
    )
    exit(1)
else:
...
print(
    bcolors.WARNING
    + "[*] Checking if the Mail-Masta endpoint is vulnerable..."
    + bcolors.ENDC
)
try:
